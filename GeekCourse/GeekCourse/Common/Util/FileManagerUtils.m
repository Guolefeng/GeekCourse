...
//  封装
//
//  Created by Eiwodetianna on 15/5/13.
...
+ (void)createResDirByDirectoryName:(NSString *)directoryName {
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL isDir = TRUE;
    BOOL isDirExist = [fileManager fileExistsAtPath:[[self class] getOtherPath:directoryName] isDirectory:&isDir];
    if (!(isDirExist && isDir)) {
        BOOL dCreateDir = [fileManager createDirectoryAtPath:[[[self class] getDocumentsPath] stringByAppendingPathComponent:directoryName] withIntermediateDirectories:YES attributes:nil error:nil];
        if (!dCreateDir) {
